...
func start_idle_bounce() -> void:
	...
	tween.set_loops()
	tween.tween_property(sprite, "scale", Vector2(1.04, 0.96), 0.35)
	tween.tween_property(sprite, "scale", Vector2(0.97, 1.03), 0.25)
	tween.tween_property(sprite, "scale", Vector2(1.0, 1.0), 0.30)
...
func _process(delta: float) -> void:
	...
	if speed < 20.0:
		speed = 0.0
	position.x += speed * delta
	if position.x >= finish_x and not has_finished:
		has_finished = true
	update_animation()
	update_particles()
func update_animation() -> void:
	...
	if speed < 20.0:
		...
		if speed < 320.0:
			...
			sprite.speed_scale = lerp(1.0, 2.0, (speed - 350.0) / 150.0)
func update_particles() -> void:
	if speed < 120.0:
		dust_particles.emitting = false
		return
	dust_particles.emitting = true
	var particle_amount := int(lerp(2.0, 8.0, speed / max_speed))
